...
class ViewController: UIViewController {
   ...
   override func viewDidLoad() {
      ...
      // 툴바 생성
      toolbar = UIToolbar(frame: CGRectMake(0, 20, self.view.frame.size.width, 44))
      ...
      // 글자 기반의 바 버튼
      let textBarButton = UIBarButtonItem(title: "문자열 버튼", style: UIBarButtonItemStyle.Plain, target: self, action: #selector(handleTextButton(_:)))
      
      // 여백
      let space = UIBarButtonItem(barButtonSystemItem: UIBarButtonSystemItem.FlexibleSpace, target: nil, action: nil)
      
      // 시스템 바 버튼
      let doneBarButton = UIBarButtonItem(barButtonSystemItem: UIBarButtonSystemItem.Done, target: self, action: #selector(handleDone(_:))) // 문자열 만으로 셀렉터 객체 생성
   ...
   override func viewWillTransitionToSize(size: CGSize, withTransitionCoordinator coordinator: UIViewControllerTransitionCoordinator) {
      ...
      self.toolbar.frame.size.width = size.width
   }
}
